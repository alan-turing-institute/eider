...
preprocess_table <- function(input_table,
  ...
  min_dates <- input_table %>% group_by(across(all_of(grouping_columns)),
                                        across(all_of(preprocess_on))) %>%
    summarise(min_date = min(!!sym(retain_min)))
  max_dates <- input_table %>% group_by(across(all_of(grouping_columns)),
                                        across(all_of(preprocess_on))) %>%
  ...
  input_table <- input_table %>%
    left_join(min_dates, by = c(grouping_columns, preprocess_on)) %>%
  ...
  input_table <- input_table %>%
    left_join(max_dates, by = c(grouping_columns, preprocess_on)) %>%
