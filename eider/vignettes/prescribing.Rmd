...
```{r}
basic_prescribing_spec_nunique_filepath <- eider_example(
  "spec_example_basic_prescribing_nunique.json"
  )
writeLines(readLines(basic_prescribing_spec_nunique_filepath)) 
```
...
tf <- transform(prescribing_data_path, basic_prescribing_spec_nunique_filepath)
...
```{r}
basic_prescribing_spec_nprescribed_filepath <- eider_example(
  "spec_example_basic_prescribing_nprescribed.json"
  )
writeLines(readLines(basic_prescribing_spec_nprescribed_filepath)) 
```

With the corresponding transformation
```{r}
tf_nitems <- transform(prescribing_data_path, 
                basic_prescribing_spec_nprescribed_filepath)
