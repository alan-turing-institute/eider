...
```{r}
with_preprocessing <- eider_example("spec_basic_preprocessing_smr04.json")
writeLines(readLines(with_preprocessing))
```

This spec is requesting a count of the number of times episode 1 or 2 from `episode_within_cis` occurs for discharge dates earlier than 31st July 2015. 

```{r}
without_preprocessing <- eider_example("spec_basic_no-preprocessing_smr04.json")
writeLines(readLines(without_preprocessing))
```
This spec has the pre-processing step removed

```{r}
smr04_data <- list(smr04 = eider_example("random_smr04_data.csv"))
with_preprocessing <- transform(
  data_sources = smr04_data,
  feature_filenames = with_preprocessing
)
with_preprocessing %>% kbl()
```


```{r}
smr04_data <- list(smr04 = eider_example("random_smr04_data.csv"))
w_out_preprocessing <- transform(
  data_sources = smr04_data,
  feature_filenames = without_preprocessing
)
w_out_preprocessing %>% kbl()
```
