...
```{r}
ae_filepath <- minisparra_example("random_ae_data.csv")
filenames <- c(ae_filepath)
all_tables <- read_all_tables(filenames)
all_tables[[ae_filepath]] %>% kbl()
```

`all_tables` is a list which associates each of the filenames with the data read from that file, and in this example it corresponds to just the random A&E data which is provided with the package.
...
```{r}
basic_ae_spec_filepath <- minisparra_example("spec_basic_ae_data.json")
