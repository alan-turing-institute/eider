...
This Quarto book serves as a demo of the minisparra package (for which we still don't have a real name). Focussing on the A&E data
...
Here we are utilising MINISPARRA's example data which is accessed via `minisparra_examples("random_ae_data.csv")`
...
`source_file` is the name for the corresponding filepath in a named list (in this case the random A&E data). `transformation_type` is the type of transformation which will take place. MINISPARRA has N different transformation types, X, Y, and Z.
...
```{r}
basic_ae_spec_filepath <- minisparra_example("spec_example_basic_ae_data.json")
...
```{r}
basic_ae_spec_w_date_filepath <- minisparra_example(
...
```{r}
basic_ae_spec_or_w_date_filepath <- minisparra_example(
