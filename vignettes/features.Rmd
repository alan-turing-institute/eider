...
As the [introduction vignette](eider.html) shows, writing R code with `eider` simply consists of a call to `run_pipeline()`.
...
They have a slightly different structure to the rest: thus, they are covered in a [separate vignette](feature_combine.html).


## Preprocessing and filtering

In addition to the keys shown above, (non-combination) features may also contain the `preprocess` and `filter` keys
