...
From the first row in this example, we can see that patient 15 was admitted on 12 June 2017.
They had 3 unique diagnosis codes: 1, 18, and 7.
...
- `source_file` specifies the identifier by which the input table is provided to `eider`;
...
The names of this list are used to match the `source_file` in the JSON file to the actual data.
In this case, because the `source_file` is `"ae"`, we need to specify the data to be used as `ae = ae_data_filepath`.
